...
public function recognitionLogs(Request $request)
{
    ...
    // New status category filter (case-insensitive, supports LIKE for wrong room)
    if ($request->status_category) {
        $cat = strtolower(trim($request->status_category));
        if ($cat === 'recognized_scheduled') {
            // recognized and scheduled, but explicitly NOT "not" and NOT "wrong room"
            ...
            // match "not scheduled" phrase or both words present
