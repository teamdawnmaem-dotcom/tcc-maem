...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Global loader */
        .loader-overlay { position: fixed; inset: 0; background: rgba(255,255,255,0.8); display: none; align-items: center; justify-content: center; z-index: 2000; flex-direction: column; gap: 12px; }
        .loader-spinner { width: 44px; height: 44px; border: 4px solid #eee; border-top-color: #8B0000; border-radius: 50%; animation: spin 0.9s linear infinite; }
...
<body>
    <div class="sidebar">
        ...
        <div class="nav-menu">
            <div class="nav-item @yield('dashboard-active')" onclick="window.location.href='{{ route('deptHead.dashboard') }}'">
            ...
            <div class="sub-nav" id="monitoring-subnav" @if(trim($__env->yieldContent('monitoring-active')) == 'active' || trim($__env->yieldContent('live-camera-active')) == 'active' || trim($__env->yieldContent('recognition-logs-active')) == 'active') style="display:flex;" @endif>
                <div class="sub-nav-item @yield('rooms-active')" onclick="window.location.href='{{ route('deptHead.room.management') }}'">Rooms</div>
                <div class="sub-nav-item @yield('cameras-active')" onclick="window.location.href='{{ route('deptHead.camera.management') }}'">Cameras</div>
                <div class="sub-nav-item @yield('live-camera-active')" onclick="window.location.href='{{ route('deptHead.live.camera.feed') }}'">Live Camera Feed</div>
            ...
            <div class="nav-item @yield('reports-active')" onclick="window.location.href='{{ route('deptHead.attendance.records') }}'">
                <span class="nav-icon" style="display:inline-flex;align-items:center;justify-content:center;">
                    <svg width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="5" width="16" height="14" rx="2" stroke="#fff" stroke-width="2"/><path d="M7 9h8M7 13h8M7 17h8" stroke="#fff" stroke-width="2"/></svg>
                </span> Reports
            </div>
        </div>
    </div>
...
<div id="accountSettingsModal" class="modal-overlay profile-modal" style="display:none;">
    <div class="modal-box">
        ...
        <form id="accountSettingsForm" class="modal-form" method="POST" action="{{ route('deptHead.account.update') }}">
            @csrf
            @method('PUT')
