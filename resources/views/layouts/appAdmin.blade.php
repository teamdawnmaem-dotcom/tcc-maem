...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .modal-btn.update { background: #7cc6fa; color: #fff; }
        .modal-btn.update:hover { background: #5bb3f5; color: #fff; }
        .modal-btn.cancel { background: #ff3636; color: #fff; }
        .modal-row {
            display: flex;
            gap: 18px;
            width: 100%;
        ...
        @media (max-width: 900px) {
            .main-content, .header { padding: 20px 10px 0 10px; }
        }
        @media (max-width: 700px) {
            .sidebar { width: 60px; }
            .main-content, .header { margin-left: 60px; }
        ...
        .loader-text { color: #8B0000; font-weight: bold; font-size: 0.95rem; }
        ...
        <div class="nav-menu">
            <div class="nav-item @yield('dashboard-active')" onclick="window.location.href='{{ route('admin.dashboard') }}'">
                <span class="nav-icon" style="display:inline-flex;align-items:center;justify-content:center;">
                    <svg width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 10.5L11 4l8 6.5V18a1 1 0 0 1-1 1h-4v-4H8v4H4a1 1 0 0 1-1-1V10.5Z" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </span> Dashboard
            </div>
            
            <div class="nav-item @yield('checker-account-active')" onclick="window.location.href='{{ route('admin.user.account.management') }}'">
                <span class="nav-icon" style="display:inline-flex;align-items:center;justify-content:center;">
                    <svg width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 7a3 3 0 1 1 6 0 3 3 0 0 1-6 0Zm8 10v-1a4 4 0 0 0-8 0v1" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </span> User Accounts
            </div>            
            ...
            <div class="nav-item has-dropdown @yield('monitoring-active') @if(trim($__env->yieldContent('monitoring-active')) == 'active' || trim($__env->yieldContent('live-camera-active')) == 'active' || trim($__env->yieldContent('recognition-logs-active')) == 'active') open @endif" onclick="toggleDropdown(this, 'monitoring')">
                <span class="nav-icon" style="display:inline-flex;align-items:center;justify-content:center;">
                    <svg width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="7" width="16" height="10" rx="2" stroke="#fff" stroke-width="2"/><circle cx="11" cy="12" r="3" stroke="#fff" stroke-width="2"/></svg>
                </span> Monitoring
            </div>
            <div class="sub-nav" id="monitoring-subnav" @if(trim($__env->yieldContent('monitoring-active')) == 'active' || trim($__env->yieldContent('live-camera-active')) == 'active' || trim($__env->yieldContent('recognition-logs-active')) == 'active') style="display:flex;" @endif>
              ...
              <div class="sub-nav-item @yield('live-camera-active')" onclick="window.location.href='{{ route('admin.live.camera.feed') }}'">Live Camera Feed</div>
              <div class="sub-nav-item @yield('recognition-logs-active')" onclick="window.location.href='{{ route('admin.recognition.logs') }}'">Recognition Logs</div> </div>
                            
                      <div class="nav-item @yield('reports-active')" onclick="window.location.href='{{ route('admin.attendance.records') }}'"> <span class="nav-icon" style="display:inline-flex;align-items:center;justify-content:center;"> <svg width="22" height="22" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="5" width="16" height="14" rx="2" stroke="#fff" stroke-width="2"/><path d="M7 9h8M7 13h8M7 17h8" stroke="#fff" stroke-width="2"/></svg> </span> Reports </div>
                ...
                <button class="profile-btn">
                    <span class="profile-icon">
                        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <circle cx="12" cy="8" r="3" stroke="white" stroke-width="2" fill="none"/>
                            <path d="M5 19c0-3.866 3.134-7 7-7s7 3.134 7 7" stroke="white" stroke-width="2" fill="none"/>
                        </svg>
                    </span>
                    <span class="profile-text">
                        <span class="profile-title">{{ auth()->user()->user_fname }} {{ auth()->user()->user_lname }}</span>
                    </span>
                    <span class="profile-chevron">&#9662;</span>
                </button>
                <div class="profile-dropdown" id="profileDropdown">
                    <div class="profile-dropdown-item" onclick="openModal('accountSettingsModal')">Account Settings</div>
                    <a href="#" class="profile-dropdown-item logout" onclick="openModal('logoutModal')">Logout</a>
                </div>
            </div>
        </div>
        </div>
...
<div id="accountSettingsModal" class="modal-overlay profile-modal" style="display:none;">
    <div class="modal-box">
        ...
        <form id="accountSettingsForm" class="modal-form" method="POST" action="{{ route('admin.account.update') }}">
            @csrf
            @method('PUT')
