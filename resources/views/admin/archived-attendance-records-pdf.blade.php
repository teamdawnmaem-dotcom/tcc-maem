...
<head>
    ...
    <style>
        ...
        /* Ensure consistent spacing for single rows */
        tbody tr:only-child {
            page-break-inside: avoid;
        }
        ...
        /* Fix summary section positioning */
        .summary-section {
            margin-bottom: 8px;
            padding: 4px 6px;
            background-color: #e8f4fd;
        ...
        /* Fix confidentiality notice positioning */
        .confidentiality-notice {
            font-size: 8px;
            color: #dc3545;
            text-align: center;
    ...
    <title>Archived Attendance Records</title>
</head>
...
<body data-generated="{{ $generatedAt->format('Y-m-d H:i:s') }}">
    ...
    <div class="filters-section">
        ...
        @if(isset($schoolYear) && $schoolYear)
            <div class="filter-item"><strong>School Year:</strong> {{ $schoolYear }}</div>
        @endif
    ...
    <table>
        <thead>
            <tr>
                <th class="archive-col">Archive Info</th>
        ...
        <tbody>
                ...
                <tr>
                    <td class="archive-col">
                        <div style="font-size: 6px;">
                            <strong>{{ $record->school_year ?? 'N/A' }}</strong><br>
                            {{ $record->semester ?? 'N/A' }}
                        </div>
                    </td>
                    <td class="date-col">{{ \Carbon\Carbon::parse($record->record_date)->format('F j, Y') }}</td>
                    <td class="name-col">{{ $record->faculty ? $record->faculty->faculty_fname . ' ' . $record->faculty->faculty_lname : 'Unknown Faculty' }}
                    </td>
                    <td class="dept-col">{{ $record->faculty ? $record->faculty->faculty_department : 'Unknown Department' }}</td>
                    <td class="course-col">{{ $record->teachingLoadArchive ? $record->teachingLoadArchive->teaching_load_course_code : 'N/A' }}</td>
                    <td class="subject-col">{{ $record->teachingLoadArchive ? $record->teachingLoadArchive->teaching_load_subject : 'N/A' }}</td>
                    <td class="class-section-col">{{ $record->teachingLoadArchive ? $record->teachingLoadArchive->teaching_load_class_section : 'N/A' }}</td>
                    <td class="day-col">{{ $record->teachingLoadArchive ? $record->teachingLoadArchive->teaching_load_day_of_week : 'N/A' }}</td>
                    <td class="time-schedule time-data">
                        @if($record->teachingLoadArchive)
                            {{ \Carbon\Carbon::parse($record->teachingLoadArchive->teaching_load_time_in)->format('h:i A') }} -<br>
                            {{ \Carbon\Carbon::parse($record->teachingLoadArchive->teaching_load_time_out)->format('h:i A') }}
                        @else
                            N/A
                        @endif
                    </td>
                    ...
                    <td class="room-col">{{ $record->camera && $record->camera->room ? $record->camera->room->room_name : 'Unknown Room' }}</td>
                    <td class="building-col">{{ $record->camera && $record->camera->room ? $record->camera->room->room_building_no : 'Unknown Building' }}</td>
                    <td class="status-col">
            ...
            @empty
                <tr>
                    <td colspan="16" style="text-align:center; padding:12px;">No records found</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="simple-footer">
        <div class="footer-left">Modern Attendance Enhance Monitoring - Archived Attendance Records Report | Generated: {{ $generatedAt->format('Y-m-d H:i:s') }}</div>
    </div>
</body>
